...
modified: 2026-09-07T06:04:46.133Z
---
...
_**Psionic Strike.**_ You can propel your weapons with psionic force. Once on each of your turns, immediately after you hit a target within 30 feet of yourself with an attack and deal damage to it with a weapon, you can expend one Psionic Energy Die, rolling it and dealing Force damage to the target equal to the number rolled plus your Intelligence modifier.
...
_**Psi-Powered Leap.**_ As a Bonus Action, you gain a Fly Speed equal to twice your Speed until the end of the current turn. Once you take this Bonus Action, you can’t do so again until you finish a Short or Long Rest unless you expend a Psionic Energy Die (no action required) to restore your use of it.
_**Telekinetic Thrust.**_ When you deal damage to a target with your Psionic Strike, you can force the target to make a Strength saving throw (DC 8 plus your Intelligence modifier and Proficiency Bonus). On a failed save, you can give the target the Prone condition or transport it up to 10 feet horizontally. ^PsiWarriorLv7

## Level 10

### Guarded Mind

You have Resistance to Psychic damage. Moreover, if you start your turn with the Charmed or Frightened condition, you can expend a Psionic Energy Die (no action required) and end every effect on yourself giving you those conditions. ^PsiWarriorLv10
...
Once you use this feature, you can’t do so again until you finish a Long Rest unless you expend a Psionic Energy Die (no action required) to restore your use of it. ^PsiWarriorLv15
...
Once you cast the spell with this feature, you can’t do so in this way again until you finish a Long Rest unless you expend a Psionic Energy Die (no action required) to restore your use of it. ^PsiWarriorLv18
...
---
const dieSize = context.bound.progression;

// JavaScript conditional checks to output custom text
if (dieSize < 5) {
    return engine.markdown.create('## Energy Dice Size: D6');
} else if (dieSize < 11) {
    return engine.markdown.create('## Energy Dice Size: D8');
} else if (dieSize < 17){
	return engine.markdown.create('## Energy Dice Size: D10');
} else {
    return engine.markdown.create('## Energy Dice Size: D12');
}
```
...
{Fighter#Level} as progression
---
const dieCount = context.bound.progression;

// JavaScript conditional checks to output custom text
if (dieCount < 5) {
    return engine.markdown.create('## Energy Dice Count: 4');
} else if (dieCount < 9) {
    return engine.markdown.create('## Energy Dice Count: 6');
} else if (dieCount < 13){
	return engine.markdown.create('## Energy Dice Count: 8');
} else if (dieCount < 17){
	return engine.markdown.create('## Energy Dice Count: 10');
} else {
    return engine.markdown.create('## Energy Dice Count: 12');
}
```
